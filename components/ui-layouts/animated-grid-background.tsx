...
export default function AnimatedGridBackground({
  ...
  return (
    ...
    >
      ...
      {mousePosition ? (
        <>
          <motion.circle cx={mousePosition.x} cy={mousePosition.y} r={width / 2} fill="none" stroke="var(--highlighted-grid-color)" strokeWidth={2} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 20 }} />
          <motion.circle cx={mousePosition.x} cy={mousePosition.y} r={width / 4} fill="var(--highlighted-grid-color)" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 260, damping: 20 }} />
        </>
      ) : null}
    </svg>
  );
